...
class SlurmEnergyTest(rfm.RunOnlyRegressionTest):
    ...
    reference = {"archer2:compute": {"energy": (0, -1, 1, "jouls")}}
    ...
    @run_after("run")
    def slurm_energy_result(self):
        """Print energy usage captured by slurm"""
        jobid= self.job.jobid
        print("jobid: ", jobid)
        slurm = rfm.utility.osext.run_command("sacct -j " + str(jobid) + " --format=User,JobID,nnodes,elapsed,ReqCPUFreq,ConsumedEnergy", check=True, shell=True)
        print("slurm: ", slurm)
        return 0


    @performance_function("jouls diff", perf_key="performance")
    def extract_perf(self):
        """Extract performance value to compare with reference value"""
        jobid= self.job.jobid
        print("jobid: ", jobid)
        slurm = rfm.utility.osext.run_command("sacct -j " + str(jobid) + " --format=JobID,ConsumedEnergy --noconvert | tr '\n' ' ' ", check=True, shell=True)
        print("slurm: ", slurm.stdout)

        energy_counters = sn.extractall(
            r"(?P<energy>[0-9]+)\sJ\s(?P<time>[0-9]+)\sus",
            self.stdout,
            "energy"
        )
