...
base_k8s_pod = {
    "apiVersion": "v1",
    "kind": "Pod",
    "metadata": {"generateName": "---CHANGE-NAME---"},
    "spec": {
        "restartPolicy": "Never",
        "containers": [
            {
                ...
                "volumeMounts": [
                    {"mountPath": "/mnt/ceph_rbd", "name": "volume"},
                    {"mountPath": "/dev/shm", "name": "devshm"},
                ],
            }
        ],
        "nodeSelector": {"nvidia.com/gpu.product": "---CHANGE-NAME---"},
        "volumes": [
            {
                "name": "volume",
                "persistentVolumeClaim": {"claimName": "---CHANGE-NAME---"},
            },
...
class DeepCAMGPUServiceBenchmark(DeepCamBaseCheck):
    ...
    def executable_setup(self):
        ...
        pod_info["spec"]["containers"][0]["args"] = [
            ...
            f"{self.lbs}",
            "-c",
            "/workspace/ML_HPC/DeepCAM/Torch/config.yaml",
            "--data_dir",
            dset_path,  # "/mnt/ceph_rbd/deepcam/mini/deepcam-data-n512" #"/mnt/ceph_rbd/gridftp-save/deepcam/All-Hist",
        ]
