...
class cbenchio_write(cbenchio):
    ...
    def set_default_parameters(self):
        ...
        self.set_default_parameter("repeat", 5)
        self.set_default_parameter("stripe_size", 1024)
    ...
    def create_write_directories(self):
            ...
            if self.stripes != 1: # Only valid on Lustre filesystem
                self.prerun_cmds.append(f"lfs setstripe -C {self.stripes} -S {self.stripe_size}K {self.path}")
    ...
    def generate_config(self):
        ...
        if self.file_per_process:
            n_elements_per_dim = int(self.field_size_per_process_per_dimension * 2**10 /( type_size **self.n_dimensions)) # Total number of elements to write per process.
        else:
            n_elements_per_dim = int(self.field_size_per_process_per_dimension * self.num_tasks * 2**10 / ( type_size **self.n_dimensions)) # Total number of elements to write if all processes write to the same file.
        ...
        for d in range(3):
            if d < self.n_dimensions:
                config["shape"][d] = n_elements_per_dim
